    ...
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            if (!forceStart)
            {
                ...
                //Display the opened WGSM
                foreach (var process in wgsm)
                {
                    if (process.Id != Process.GetCurrentProcess().Id)
                    {
                        System.Media.SystemSounds.Beep.Play();
                        MessageBox.Show("Another instance is already running", "WGSM already running", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            AppDomain.CurrentDomain.UnhandledException += async (s, args) =>
            {
                ...
                File.AppendAllText(logFile, $"WGSM v{version}\n\n" + args.ExceptionObject);
#if !DEBUG
                string latestLogFile = Path.Combine(logPath, "latest_crash_wgsm_temp.log");
                File.AppendAllText(latestLogFile, $"WGSM v{version}\n\n" + args.ExceptionObject);
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\WGSM");
                if (key != null && (key.GetValue("RestartOnCrash") ?? false).ToString() == "True")
                {
                    Process p = new Process
                    {
                        StartInfo =
                        {
                            ...
                            FileName = "cmd.exe",
                            Arguments = "/C echo WGSM will auto restart after 10 seconds... & echo Close this windows to cancel... & ping 0 -w 1000 -n 10 > NUL & start WGSM.exe /ForceStart /ShowCrashHint",

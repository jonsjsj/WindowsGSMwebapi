...
namespace WindowsGSM
{
    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        public MainWindow(bool showCrashHint)
        {
            ...
            var key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\WGSM");
            if (key == null)
            {
                key = Registry.CurrentUser.CreateSubKey(@"SOFTWARE\WGSM");
...
#pragma warning restore
            ...
            {
                BalloonTipTitle = "WGSM",
                BalloonTipText = "WGSM is running in the background",
                Text = "WGSM",
                BalloonTipIcon = ToolTipIcon.Info,
                Visible = true
            };

            notifyIcon.Icon = new System.Drawing.Icon(System.Windows.Application.GetResourceStream(new Uri("pack://application:,,,/Images/WGSM-Icon.ico")).Stream);
            ...
            if (showCrashHint)
            {
                ...
                Log("System", $"WGSM crashed unexpectedly, please view the crash log {logFile}");
            }
        ...
        public void AddGamesToComboBox()
        {
            ...
            gameName.ForEach(delegate (DictionaryEntry entry) { sortedList.Add(entry.Key, $"/WGSM;component/{entry.Value}"); });
            int pluginLoaded = 0;
            PluginsList.ForEach(delegate (PluginMetadata plugin)
            {
                ...
                {
                    pluginLoaded++;
                    sortedList.Add(plugin.FullName, plugin.GameImage == PluginManagement.DefaultPluginImage ? plugin.GameImage.Replace("pack://application:,,,", "/WGSM;component") : plugin.GameImage);
                }
            });
        ...
        private async void ImportPlugin_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("WGSM is currently installing/importing server!", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            if (dr == System.Windows.Forms.DialogResult.OK)
            {
                ...
                /// This is relying on it keeps the naming shceme of the ZIP files that're downloaed from GitHub releases. Like WGSM.Spigot-1.0.zip,
                /// Just by following WGSM naming of plugins, and this will be fine!
        ...
        public void LoadServerTable()
        {
            ...
            for (int i = 1; i <= MAX_SERVER; i++)
            {
                ...
                string configpath = ServerPath.GetServersConfigs(i.ToString(), "WGSM.cfg");
                ...
                try
                {
                        ...
                        PluginsList.ForEach(delegate (PluginMetadata plugin)
                        {
                            if (plugin.FullName == serverConfig.ServerGame && plugin.IsLoaded)
                            {
                                icon = plugin.GameImage == PluginManagement.DefaultPluginImage
                                    ? plugin.GameImage.Replace("pack://application:,,,", "/WGSM;component")
                                    : plugin.GameImage;
                            }
                        });
                    }
                    if (icon == null)
                    {
                        icon = PluginManagement.DefaultPluginImage.Replace("pack://application:,,,", "/WGSM;component");
                    }
        ...
        private void Window_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            using (var key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\WGSM", true))
            {
        ...
        private void Install_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!progressbar_InstallProgress.IsIndeterminate)
            {
                ...
                textbox_InstallServerName.Text = $"WGSM - Server #{newServerConfig.ServerID}";
            }
        }
        ...
        private async void Button_Install_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (gameServer.IsInstallValid())
            {
                ...
                newServerConfig.CreateWGSMConfig();

                // Create WGSM.cfg and game server config
                try
                {
        ...
        private void Import_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!progressbar_ImportProgress.IsIndeterminate)
            {
                ...
                var newServerConfig = new ServerConfig(null);
                textbox_ImportServerName.Text = $"WGSM - Server #{newServerConfig.ServerID}";
            }
        }
        ...
        private async Task<bool> GameServer_Backup(ServerTable server, string notes = "")
        {
            ...
            string tempRoot = Path.Combine(Path.GetTempPath(), "WGSM_Backup", server.ID, timestamp);
            try
            {
        ...
        private async Task<bool> GameServer_RestoreBackup(ServerTable server, string backupFile)
        {
            ...
            string tempRoot = Path.Combine(Path.GetTempPath(), "WGSM_Backup_Restore", server.ID, DateTime.Now.ToString("yyyyMMddHHmmss"));
            string error = string.Empty;
        ...
        private async Task<bool> GameServer_Delete(ServerTable server)
        {
            ...
            if (Directory.Exists(serverPath))
            {
                string wgsmCfgPath = ServerPath.GetServersConfigs(server.ID, "WGSM.cfg");
        ...
        private async Task EndAllRunningProcess(string serverId)
        {
            await Task.Run(() =>
            {
                //LINQ query for wgsm old processes
        ...
        {
            Process.Start(new ProcessStartInfo("https://wgsm.com/") { UseShellExecute = true });
        }
        ...
        {
            Process.Start(new ProcessStartInfo("https://www.patreon.com/WGSM/") { UseShellExecute = true });
        }
        ...
        {
            RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\WGSM", true);
            ...
            key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\WGSM", true);
            string authKey = (key.GetValue(RegistryKeyName.DonorAuthKey) == null) ? string.Empty : key.GetValue(RegistryKeyName.DonorAuthKey).ToString();
            ...
            else
            {
                key.SetValue(RegistryKeyName.DonorTheme, "False");
                key.SetValue(RegistryKeyName.DonorAuthKey, "");
                await this.ShowMessageAsync("Fail to activate.", "Please visit https://wgsm.com/patreon/ to get the key.");
        ...
        private async Task<(bool, string)> AuthenticateDonor(string authKey)
        {
            try
            {
                ...
                {
                    string json = await webClient.DownloadStringTaskAsync($"https://wgsm.com/patreon/patreonAuth.php?auth={authKey}");
        ...
        {
            Process.Start(new ProcessStartInfo("https://docs.wgsm.com") { UseShellExecute = true });
        }

        private void Help_ReportIssue_Click(object sender, RoutedEventArgs e)
        {
            Process.Start(new ProcessStartInfo("https://github.com/raziel7893/WGSM/issues") { UseShellExecute = true });
        }
        ...
        private async void Help_SoftwareUpdates_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                await this.ShowMessageAsync("Software Updates", "WGSM is up to date.");
                return;
            }
        ...
        private async Task<string> GetLatestVersion()
        {
            try
            {
                var webRequest = WebRequest.Create("https://api.github.com/repos/jonsjsj/WGSMwebapi/releases/latest") as HttpWebRequest;
        ...
        private async Task<string> DownloadLatestInstaller(string tag)
        {
            ...
            string fileName = $"WGSM-WebAPI-Full-Setup-{version}.exe";
            string url = $"https://github.com/jonsjsj/WGSMwebapi/releases/download/{tag}/{fileName}";
        ...
        private async void Help_AboutWGSM_Click(object sender, RoutedEventArgs e)
        {
            var settings = new MetroDialogSettings
            {
            ...
            var result = await this.ShowMessageAsync("About WGSM", $"Product:\t\tWGSM\nVersion:\t\t{WGSM_VERSION.Substring(1)}\nCreator:\t\tTatLead\n\nIf you like WGSM, consider becoming a Patron!", MessageDialogStyle.AffirmativeAndNegative, settings);
        ...
        */
        ...
        {
            using var key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\WGSM");

...
namespace WindowsGSM.Functions
{
    public class ServerConfig
    {
        ...
        public ServerConfig(string serverid)
        {
            ...
            if (string.IsNullOrEmpty(serverid))
            {
                for (int id = 1; id <= MainWindow.MAX_SERVER; id++)
                {
                    ...
                    {
                        string config = MainWindow.WGSM_PATH + @"\servers\" + id + @"\configs\WGSM.cfg";
            ...
            string configpath = ServerPath.GetServersConfigs(serverid, "WGSM.cfg");
        ...
        public string GetAvailablePort(string defaultport, int increment)
        {
            ...
            int[] portlist = new int[WGSM.ServerGrid.Items.Count];

            for (int i = 0; i < WGSM.ServerGrid.Items.Count; i++)
            {
                ServerTable row = WGSM.ServerGrid.Items[i] as ServerTable;
        ...
        {
            string configFile = ServerPath.GetServersConfigs(serverId, "WGSM.cfg");

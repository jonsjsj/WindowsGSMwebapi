...
namespace WindowsGSM.Functions
{
    class DiscordWebhook
    {
        ...
        {
            string url = "https://github.com/WGSM/Discord-Alert-Icons/raw/master/";
            if (serverStatus.Contains("Started"))
            {
        ...
        private static string GetServerGameIcon(string serverGame)
        {
            try
            {
                return @"https://github.com/WGSM/WGSM/raw/master/WGSM/" + GameServer.Data.Icon.ResourceManager.GetString(serverGame);
            }
            catch
            {
                return @"https://github.com/WGSM/WGSM/raw/master/WGSM/Images/WGSM.png";
            }
        }

        private string GetAvatarUrl()
        {
            return "https://github.com/WGSM/WGSM/raw/master/WGSM/Images/WGSM" + (string.IsNullOrWhiteSpace(_donorType) ? string.Empty : $"-{_donorType}") + ".png";
        }
        ...
        private static async Task SendErrorLogToDiscord(string errorLog)
        {
            try
            {
                ...
                {
                    { "username", "WGSM - Error Feed" },
                    { "avatar_url", "https://github.com/WGSM/WGSM/raw/master/WGSM/Images/WGSM.png" },
                    { "content",  $"```php\n{errorLog}```" }
                };

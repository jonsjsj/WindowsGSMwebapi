...
namespace WGSM.Plugins
{
    ...
    public class SDTD_ExperimentalBeta : SteamCMDAgent
    {
        ...
        {
            name = "WGSM.SDTD_ExperimentalBeta",
            author = "LegendSeeker",
            description = "🧩 WGSM plugin for supporting 7 Days to Die Dedicated Server running Experimental Beta",
            version = "0.1",
            url = "https://github.com/LegendSeeker/WGSM.SDTD_ExperimentalBeta",
            ...
            if (await Functions.Github.DownloadGameServerConfig(configPath, FullName))
            {
                ...
                while (line != null)
                {
                    if (line.Contains("{{hostname}}"))
                        ...
                        //it is not good to have the serverdata in %AppData% as the user will forget it, as nearly all wgsm servers store it inside the WindosGSM structure
        ...
        public async Task<Process> Start()
        {
            ...
            {
                Notice = $"serverconfig.xml not found ({configPath}), reloading it from https://github.com/WGSM/Game-Server-Configs";
                await DoCreateConfig();
                if (!File.Exists(configPath))
            ...
            if (Directory.Exists(OldProfile))
            {
                Notice = Notice + $"An Old Savegame found in {OldProfile}\n\n" +
                        $"If you want to have it Inside WGSM to have it all in one place move it to\n" +
